...
## Философия (НЕ нарушать — детали в ROADMAP «Принципы продукта»)
...
- **Навигация:** реестр `SECTIONS` + `renderNav()` + `sectionOf()` + `navUnlockLevel()` (2 уровня: разделы + саб-табы; на телефоне — **bottom-nav**). Гейт-уровни (Герой/Племя с ур.3). `NEW_VIEWS` + `settings.discovered` = glow-подсветка новых разделов.
...
| **STYLE-DECISION.md** | финал арт-стиля (painterly-cutout 2D) + анимация (paper-doll в коде) |
