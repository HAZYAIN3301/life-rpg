...
**Browser Companion Discovery v200:** существующий пользователь видит временное объявление на Today с `установить / через 3 дня / больше не напоминать`; новый — только после 24 часов и следующего активного входа. Есть отдельная открытая install-page, трёхшаговый modal, dedicated icon/badge и готовый Chrome Web Store upload package. Реальная установка в один клик появится только после публикации владельцем в store. QA/handoff: [`BROWSER-COMPANION-DISCOVERY-V200-QA.md`](./BROWSER-COMPANION-DISCOVERY-V200-QA.md).

**Interface Hierarchy v203:** header больше не занят постоянной полосой сфер; ядро дня, Shadow rail, Today/Calendar forms, Inspiration references и Board Wildcard используют одну понятную иерархию «выбор → параметры». Переключение ядра fail-closed и не показывает успех до durable task write. Движение конечное и optional, звук семантический, touch/keyboard/reduced-motion контракты сохранены. Локальный QA/handoff: [`INTERFACE-HIERARCHY-V203-QA.md`](./INTERFACE-HIERARCHY-V203-QA.md); production release ещё не выполнялся.
