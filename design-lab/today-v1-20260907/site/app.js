...
function render(){
  ...
  $('#tasks').innerHTML=tasks.length?tasks.map(taskHTML).join(''):`<div class="empty-state"><h3>Есть место для твоих планов.</h3><p>Начни с одного дела. Время можно выбрать сразу — или оставить свободным.</p><button class="primary" id="empty-add">Добавить первое дело</button></div>`;
...
function openDialog(title,html){opener=document.activeElement;$('#dialog-title').textContent=title;$('#dialog-body').innerHTML=html;if(!$('#dialog').open)$('#dialog').showModal();audio?.play('open');}
...
function closeDialog(){if((recorder?.state==='recording'||(audioUrl&&!audioDownloaded))&&!confirm('Запись ещё не скачана. Закрыть и убрать её?'))return;cleanupMic();$('#dialog').close();opener?.isConnected&&opener.focus();audio?.play('close');}
...
document.addEventListener('click',e=>{
  ...
  if(target.dataset.toggle){const id=target.dataset.toggle;const wasDone=state.tasks.find(t=>t.id===id)?.done;if(commit({type:'toggle',id},wasDone?'Выполнение снято':'Готово. Задача выполнена.',{sound:wasDone?'select':'complete'}))document.querySelector(`[data-task="${CSS.escape(id)}"]`)?.classList.add('just-completed');}
  ...
  else if(target.id==='empty-add')$('#task-name').focus();
  else if(target.id==='view-notes'||target.dataset.section==='Заметки')showNotes();
  else if(target.dataset.section==='Привычки')$('.habits-strip').scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth',block:'center'});
...
$('#composer').onsubmit=e=>{e.preventDefault();const data=Object.fromEntries(new FormData(e.currentTarget));const id=crypto.randomUUID();if(commit({type:'add',task:{...data,id,date:state.day,done:false,core:!dayTasks(state).length}},'Задача добавлена',{sound:'confirm'})){$('#task-name').value='';$('#task-name').focus();}};
