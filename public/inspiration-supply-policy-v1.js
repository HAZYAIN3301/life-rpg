...
(function exposeInspirationSupplyPolicy(root, factory) {
  ...
  const LANGS = Object.freeze(['ru', 'en', 'de', 'uk', 'es', 'none']);
  ...
  const PLAYBACK_PROOFS = Object.freeze(['playback', 'manual']);
  ...
  function admit(rawCandidates, rawCtx) {
    ...
    for (const raw of rawCandidates) {
      ...
      if (c.tier !== 'licensed-local') {
        ...
        if (PLAYBACK_PROOFS.indexOf(c.checkMethod) < 0) {
  ...
  function eligibleToday(items, options) {
    ...
    for (const c of Array.isArray(items) ? items : []) {
      ...
      if (c.lang !== 'none' && locales.indexOf(c.lang) < 0) { blocked.push({ id: c.id, code: BLOCK.LANGUAGE, detail: c.lang }); continue; }
      survivors.push(c);
    }
  ...
  function fromCatalogV1Row(row, options) {
    ...
    const policy = row.mediaPolicy === 'iframe' ? 'embed' : row.mediaPolicy === 'link' ? 'link'
      : row.mediaPolicy === 'remote-image' ? 'link' : row.rightsKind === 'satoru-original' ? 'local' : null;
    if (!policy) missing.push(`mediaPolicy:${row.mediaPolicy}`);

    const seconds = durationLabelToSeconds(row.durationLabel);
    ...
    return {
      ...
      draft: Object.freeze({
        ...
        rights: { kind, holder: row.attribution, url: row.rightsUrl, embedAllowed: policy === 'embed', downloadAllowed: kind === 'satoru-original' },
        delivery: { policy, embedUrl: row.embedUrl, sourceUrl: row.sourceUrl },
    ...
    return (Array.isArray(items) ? items : []).map((c) => Object.freeze({
      ...
      mediaPolicy: c.delivery.policy === 'embed' ? 'iframe'
        : c.delivery.policy === 'local' ? 'local'
          : c.delivery.policy === 'text' ? 'text' : 'link',
      embedUrl: c.embedUrl || '',
