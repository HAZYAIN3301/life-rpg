...
function openSphereGuide(name) {
  ...
  const body = g
    ? `<p class="sg-what">${esc(g.what)}</p>
       <div class="sg-row"><b>✅ ${t('Что засчитывать')}</b><span>${esc(g.count)}</span></div>
       <div class="sg-row"><b>📈 ${t('Чем развивать')}</b><span>${esc(g.grow)}</span></div>`
