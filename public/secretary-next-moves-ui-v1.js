...
(function (root, factory) {
  ...
  const rows = {
    ...
    'morning.stale': ['Ответ сохранён. Утренний шаг уже устарел; обнови предложение.', 'Your response is saved. The morning step has expired; refresh the offer.', 'Deine Antwort ist gespeichert. Der Morgenschritt ist nicht mehr aktuell; aktualisiere den Vorschlag.', 'Відповідь збережена. Ранковий крок уже застарів; онови пропозицію.', 'Tu respuesta está guardada. El paso matutino ha caducado; actualiza la propuesta.'],
    'morning.terminal': ['Это утреннее предложение уже закрыто или недоступно. Обнови предложение.', 'This morning offer is closed or unavailable. Refresh the offer.', 'Dieser Morgenvorschlag ist abgeschlossen oder nicht verfügbar. Aktualisiere den Vorschlag.', 'Ця ранкова пропозиція вже закрита або недоступна. Онови пропозицію.', 'Esta propuesta matutina está cerrada o no disponible. Actualiza la propuesta.'],
    'morning.storage': ['Не удалось прочитать сохранённый ответ в этой вкладке. Повтори проверку или продолжи без его повтора.', 'The saved response in this tab could not be read. Retry the check or continue without replaying it.', 'Die gespeicherte Antwort in diesem Tab konnte nicht gelesen werden. Prüfe erneut oder fahre ohne Wiederholung fort.', 'Не вдалося прочитати збережену відповідь у цій вкладці. Повтори перевірку або продовж без її повтору.', 'No se pudo leer la respuesta guardada en esta pestaña. Vuelve a comprobarlo o continúa sin reenviarla.'],
    'morning.continue': ['Продолжить без повтора', 'Continue without replay', 'Ohne Wiederholung fortfahren', 'Продовжити без повтору', 'Continuar sin reenviar'],
  ...
  function errorHTML(error, lang, retry = true) {
    const morningKey = { morning_stale: 'morning.stale', morning_terminal: 'morning.terminal', morning_storage: 'morning.storage' }[error];
    const key = morningKey || (error === 'stale_evening' ? 'evening.changed' : /invalid_secretary|invalid_ledger|corrupt/.test(error || '') ? 'common.corrupt' : /stale_target|offer_expired|terminal_outcome|offer_not_found/.test(error || '') ? 'common.stale' : error === 'context_blocked' ? 'common.blocked' : error === 'opening_deferred' ? 'common.deferred' : 'common.error');
