...
(function exposeGuideV3CopyRu(root, factory) {
  ...
  const COPY = Object.freeze({
    ...
    "chapter.tree.title": "Карта развития",
    ...
    "context.tree.prompt": "Здесь два разных слоя. «Путь» хранит реальные результаты: ближайшую веху, её критерий и следующий шаг. «Игровые бонусы» меняют только Satoru и не доказывают навык. Открой выделенную сферу и сначала посмотри её Путь.",
    "context.tree.complete": "Перед тобой ближайшая реальная веха. Критерий показывает, что считается результатом. Если следующий шаг уже настроен, его можно добавить в план; если нет — сначала уточнить с Тенью. Фиксируй веху только когда результат уже есть. Игровые бонусы находятся отдельно и не заменяют подтверждение.",
