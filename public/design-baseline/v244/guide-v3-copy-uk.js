...
(function exposeGuideV3CopyUk(root, factory) {
  ...
  const COPY = Object.freeze({
    ...
    "chapter.tree.title": "Карта розвитку",
    "chapter.stats.title": "Прогрес",
    ...
    "context.hero.prompt": "Твій Герой показує доведений прогрес: рівень, ранг і форму. Тут немає окремої сили, яку треба фармити заради картинки.",
    "context.hero.complete": "Рівень не згорає. Гардероб змінює лише те, що ти сам обираєш.",
    ...
    "context.pets.prompt": "Кожен улюбленець пов'язаний з основною сферою. Завершені дії годують його, а недавній ритм змінює стан. Це спосіб помітити перекіс, а не оцінка чи борг.",
    "context.pets.complete": "Підказка улюбленця показує, що зараховується до його сфери. Виправляти все одразу не треба.",

    "context.tree.prompt": "Тут є два окремі шари. «Шлях» зберігає реальні результати: найближчу віху, її критерій і наступний крок. «Ігрові бонуси» змінюють лише Satoru й не доводять навичку. Відкрий виділену сферу й спочатку подивись її Шлях.",
    "context.tree.complete": "Перед тобою найближча реальна віха. Критерій показує, що вважається результатом. Якщо наступний крок уже налаштований, його можна додати до плану; якщо ні — спочатку уточнити з Тінню. Фіксуй віху лише тоді, коли результат уже є. Ігрові бонуси розташовані окремо й не замінюють підтвердження.",

    "context.stats.prompt": "Назбиралося сім днів активності — вже можна побачити ритм без здогадок. Подивись один графік.",
    "context.stats.complete": "Прогрес показує спостереження, а не оцінку тебе. Рішення все одно залишається за тобою.",
