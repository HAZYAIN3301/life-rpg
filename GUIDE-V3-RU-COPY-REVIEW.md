...
> Статус: **тон утверждён Альбертом 2026-08-24** — голос Тени переписан на 11 отмеченных ключах,
> см. журнал правки ниже.
>
> Это не утверждённая runtime-copy.
> До утверждения этого файла реплики нельзя переводить на EN/DE/UK/ES.
>
> Формально ничего не изменилось: `RUNTIME_APPROVED` осознанно оставлен `false`. Флаг переключает
> разом и локальный запуск гайда для RU-пользователей, и старт перевода
> (`scripts/guide-v3-runtime.test.js`, тест «unapproved RU draft cannot auto-start»), поэтому сам
> флип — отдельное решение по готовности, а не механический шаг переноса текста. Следующий ход —
> Codex: перевести EN → DE (приоритет 18.08) и затем осознанно поднять флаг вместе с visual
> QA-гейтами.
> Runtime gate: `GuideV3CopyRu.RUNTIME_APPROVED === false`.

Источник истины для кода: `public/guide-v3-copy-ru.js`. Ниже находится точное зеркало всех 143 ключей версии `0.2.0-draft`.
...
- **`first.release`** — из авторского текста вырезана вставка `(СООБЩЕНИЕ ДЛЯ CLAUDE CODE — ...)`:
  ...
  шагах First Journey) — технически это наведение того же spotlight на `.help-btn`, а не новая
  механика. Это задача Codex в `app.js`/`guide-v3.js`, не сделана.
...
- **⚠️ `context.rewards.choose`** — авторский текст дословно повторяет «закон равноценного обмена»
  из Fullmetal Alchemist (аниме/манга), без атрибуции, как якобы собственные слова Тени. Это
  копирайт-риск при показе в живом продукте с Pro-тиром: узнаваемая фраза персонажа, выданная за
  оригинальный текст приложения. Перенесено как есть, ничего не меняю без прямого решения — но
  флагаю явно. Если решишь оставить — на твой риск и с открытыми глазами; альтернатива — та же
  мысль своими словами, без узнаваемой формулировки.
