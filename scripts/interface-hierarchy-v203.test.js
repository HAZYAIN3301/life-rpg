...
test('core quest is labelled and raised without reintroducing a checkbox stripe', () => {
  ...
  assert.ok(row.includes("${satoruIconHTML('system.focus', 'inline-glyph')}${esc(t('Ядро дня'))}"));
...
test('v210 Browser Protection shell advances coherently without repinning unchanged questionnaire code', () => {
  assert.match(SW, /const CACHE = 'satoru-v297'/);
  assert.match(APP, /const PWA_CACHE_VERSION = 'satoru-v297'/);
  assert.match(INDEX, /return-shelf-ui-v1\.js\?v=20260919-inspiration-v265-1/);
  assert.match(INDEX, /styles\.css\?v=20260926-extension-v297-1/);
  assert.match(INDEX, /questionnaire-v1\.js\?v=20260830-browser-companion-v206-1/);
  assert.match(INDEX, /app\.js\?v=20260926-extension-v297-1/);
});
