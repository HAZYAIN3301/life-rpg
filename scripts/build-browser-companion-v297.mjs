...
if (manifest.version !== '0.7.0') throw new Error('v297 expects extension version 0.7.0');
const runtime = readdirSync(source).filter(name => !name.endsWith('.test.js') && /\.(js|html|css|png)$/.test(name));
for (const needed of ['adult-list.js']) if (!runtime.includes(needed)) throw new Error(`missing ${needed}`);
runtime.push('manifest.json', 'THIRD-PARTY-NOTICES.md', 'rules/adult-redirect.json', 'rules/adult-block.json', 'rules/LICENSE-OISD.txt');
for (const locale of readdirSync(join(source, '_locales')).sort()) runtime.push(`_locales/${locale}/messages.json`);
runtime.sort();
const staging = mkdtempSync(join(tmpdir(), 'satoru-attention-v297-'));
...
try {
  ...
  const primary = join(downloads, 'satoru-attention-chromium-v297.zip');
  ...
  const outputs = ['satoru-attention-chromium-v297.zip'];
  const receipt = { release: 'v297', extensionVersion: manifest.version, target: 'Chrome and Brave desktop', published: false,
  ...
  mkdirSync(join(source, 'store-kit-v297'), { recursive: true });
  writeFileSync(join(source, 'store-kit-v297/release.json'), `${JSON.stringify(receipt, null, 2)}\n`);
