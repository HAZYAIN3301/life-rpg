'use strict';
// v299: Satoru Attention 0.8.0 — merged adult list (OISD, HaGeZi, StevenBlack, Satoru) and the Reddit NSFW guard.
...
const receipt = JSON.parse(fs.readFileSync(path.join(ext, 'store-kit-v299/release.json')));
const hash = bytes => createHash('sha256').update(bytes).digest('hex');

test('v299 package is exact-source, carries the merged rulesets, the Reddit guard and every license', () => {
  assert.equal(receipt.release, 'v299'); assert.equal(receipt.extensionVersion, '0.8.0'); assert.equal(receipt.published, false);
  assert.deepEqual(receipt.outputs.map(item => item.path), ['public/downloads/satoru-attention-chromium-v299.zip']);
...
test('the site offers the v299 package and no longer precaches the ZIP', () => {
  const app = fs.readFileSync(path.join(root, 'public/app.js'), 'utf8');
  assert.match(app, /const BROWSER_COMPANION_DOWNLOAD = 'downloads\/satoru-attention-chromium-v299\.zip';/);
  assert.match(fs.readFileSync(path.join(root, 'public/browser-companion.html'), 'utf8'), /href="downloads\/satoru-attention-chromium-v299\.zip"/);
  const landing = fs.readFileSync(path.join(root, 'public/browser-companion-landing-v1.js'), 'utf8');
  assert.equal((landing.match(/downloads\/satoru-attention-chromium-v299\.zip/g) || []).length, 5);
  assert.doesNotMatch(landing, /v2(60|97)\.zip/);
  assert.doesNotMatch(fs.readFileSync(path.join(root, 'public/sw.js'), 'utf8'), /downloads\//);
  assert.match(fs.readFileSync(path.join(root, 'public/browser-companion-privacy-v1.js'), 'utf8'), /Extension 0\.8\.0/);
