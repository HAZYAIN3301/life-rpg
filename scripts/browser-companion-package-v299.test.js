...
test('v299 package is exact-source, carries the merged rulesets, the Reddit guard and every license', () => {
  ...
  // Since v300 the live sources are compared with the v300 package; v299 stays byte-exact to its receipt.
  for (const item of receipt.runtime) assert.equal(hash(execFileSync('unzip', ['-p', zip, item.path], { maxBuffer: 64 * 1024 * 1024 })), item.sha256);
