...
test('lifecycle reducer prioritizes offline, update and explicit reconnect recovery', () => {
  let state = PWA.create({ currentVersion: 'satoru-v189', online: true });
  ...
  state = PWA.reduce(state, { type: 'worker:version', version: 'satoru-v190' });
...
test('invalid messages and forged cache versions fail closed without throwing', () => {
  const state = PWA.create({ currentVersion: 'satoru-v189' });
  ...
  assert.equal(PWA.cacheVersion('satoru-v189'), 'satoru-v189');
  assert.equal(PWA.cacheVersion(' satoru-v189 '), 'satoru-v189');
});

test('same worker version is quiet and refresh cannot start offline', () => {
  let state = PWA.create({ currentVersion: 'satoru-v189', online: false });
  state = PWA.reduce(state, { type: 'worker:version', version: 'satoru-v189' });
...
test('service worker install is fail-closed and activated version reaches every open client', () => {
  assert.match(SW, /const CACHE = 'satoru-v189'/);
...
test('runtime exposes an accessible update/offline surface and fences writes while offline', () => {
  assert.match(APP, /const PWA_CACHE_VERSION = 'satoru-v189'/);
...
test('five-language lifecycle copy and v181 shell order are complete', () => {
  ...
  const appAt = INDEX.indexOf('app.js?v=20260827-motion-sound-v189-1');
  assert.ok(moduleAt >= 0 && appAt > moduleAt);
  assert.match(INDEX, /styles\.css\?v=20260827-motion-sound-v189-1/);
