...
test('lifecycle reducer prioritizes offline, update and explicit reconnect recovery', () => {
  let state = PWA.create({ currentVersion: 'satoru-v180', online: true });
  ...
  state = PWA.reduce(state, { type: 'worker:version', version: 'satoru-v181' });
...
test('invalid messages and forged cache versions fail closed without throwing', () => {
  const state = PWA.create({ currentVersion: 'satoru-v180' });
  ...
  assert.equal(PWA.cacheVersion('satoru-v180'), 'satoru-v180');
  assert.equal(PWA.cacheVersion(' satoru-v180 '), 'satoru-v180');
});

test('same worker version is quiet and refresh cannot start offline', () => {
  let state = PWA.create({ currentVersion: 'satoru-v180', online: false });
  state = PWA.reduce(state, { type: 'worker:version', version: 'satoru-v180' });
