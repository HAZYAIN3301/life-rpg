...
test('ни одна из 11 существующих строк каталога ещё не удовлетворяет схеме поставки', () => {
  ...
  for (let i = 0; i < missingByRow.length; i += 1) {
    ...
    assert.equal(res.ok, false, `${Catalog.CATALOG[i].id} внезапно полон — обнови INSPIRATION-SUPPLY-V1.md`);
    for (const field of ['lang', 'lastCheckedAt', 'available']) {
      assert.ok(res.missing.includes(field), `${Catalog.CATALOG[i].id}: ожидалось отсутствие ${field}`);
    }
  }
