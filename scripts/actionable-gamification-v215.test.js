...
test('v215 integrity modules remain ordered while the app shell advances to v247', () => {
  ...
  assert.match(SW, /const CACHE = ['"]satoru-v247['"]/, 'service-worker cache must be bumped to satoru-v247');
  ...
  assert.match(APP, /const PWA_CACHE_VERSION = ['"]satoru-v247['"]/, 'app and service worker disagree on the v247 cache');
  ...
  assert.match(scriptTag('app.js'), /[?&](?:v|build)=[^"']*v247(?:[-_.][^"']*)?["']/i,
    'app.js query does not identify the v247 release');
});
