...
test('v215 integrity modules remain ordered while the app shell advances to v248', () => {
  ...
  assert.match(SW, /const CACHE = ['"]satoru-v260['"]/, 'service-worker cache must be bumped to satoru-v260');
  ...
  assert.match(APP, /const PWA_CACHE_VERSION = ['"]satoru-v260['"]/, 'app and service worker disagree on the v248 cache');
  ...
  assert.match(scriptTag('app.js'), /[?&](?:v|build)=[^"']*v260(?:[-_.][^"']*)?["']/i,
    'app.js query does not identify the v260 release');
});
