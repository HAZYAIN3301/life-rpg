...
test('v215 integrity modules remain ordered while the app shell advances to v248', () => {
  ...
  assert.match(SW, /const CACHE = ['"]satoru-v276['"]/, 'service-worker cache must be bumped to satoru-v276');
  ...
  assert.match(APP, /const PWA_CACHE_VERSION = ['"]satoru-v276['"]/, 'app and service worker disagree on the v248 cache');
  ...
  assert.match(scriptTag('app.js'), /[?&](?:v|build)=[^"']*v276(?:[-_.][^"']*)?["']/i,
    'app.js query does not identify the v276 release');
});
