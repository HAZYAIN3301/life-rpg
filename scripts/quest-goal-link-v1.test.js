...
test('the module is loaded before app.js and cached once for offline', () => {
  ...
  assert.match(SW, /const CACHE = 'satoru-v252'/, 'новый файл в SHELL обязан поднять версию кэша');
  assert.match(APP, /const PWA_CACHE_VERSION = 'satoru-v252'/);
});
...
test('the day names the goal on the quest row itself, not only inside «•••»', () => {
  ...
  assert.match(APP, /<div class="t-title">\$\{coreBadge\}\$\{titleControl\}<div class="task-context">\$\{skSel\}\$\{questGoalChipHTML\(q, links\)\}/);
