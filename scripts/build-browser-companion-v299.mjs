...
if (manifest.version !== '0.8.0') throw new Error('v299 expects extension version 0.8.0');
...
runtime.sort();
const staging = mkdtempSync(join(tmpdir(), 'satoru-attention-v299-'));
...
try {
  ...
  const primary = join(downloads, 'satoru-attention-chromium-v299.zip');
  ...
  const outputs = ['satoru-attention-chromium-v299.zip'];
  const receipt = { release: 'v299', extensionVersion: manifest.version, target: 'Chrome and Brave desktop', published: false,
  ...
  mkdirSync(join(source, 'store-kit-v299'), { recursive: true });
  writeFileSync(join(source, 'store-kit-v299/release.json'), `${JSON.stringify(receipt, null, 2)}\n`);
