...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v290';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*today-v290(?:-|$)/, 'the changed app shell needs the v290 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*today-v290(?:-|["'])/,
    'the current application CSS needs the v290 cache-busting pin');
});
