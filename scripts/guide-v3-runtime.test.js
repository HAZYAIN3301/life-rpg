...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v273';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*release-qa-v273(?:-|$)/, 'the changed app shell needs the v273 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*release-qa-v273(?:-|["'])/,
    'the current application CSS needs the v273 cache-busting pin');
});
