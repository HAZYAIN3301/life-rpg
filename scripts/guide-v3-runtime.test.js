...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v270';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*search-v270(?:-|$)/, 'the changed app shell needs the v266 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*search-v270(?:-|["'])/,
