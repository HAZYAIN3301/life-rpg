...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v207';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*v207(?:-|$)/, 'the current app shell needs the v207 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*v207(?:-|["'])/,
    'the current application CSS needs the v207 cache-busting pin');
});
