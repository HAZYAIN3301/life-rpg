...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v257';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*inspiration-v257(?:-|$)/, 'the changed app shell needs the v257 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*secretary-v256(?:-|["'])/,
    'the current application CSS needs the v256 cache-busting pin');
});
