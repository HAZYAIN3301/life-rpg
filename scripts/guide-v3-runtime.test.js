...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v300';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*lock-v300(?:-|$)/, 'the changed app shell needs the v300 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*lock-v300(?:-|["'])/,
    'the current application CSS needs the v300 cache-busting pin');
});
