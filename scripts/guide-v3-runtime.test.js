...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v272';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*time-field-v272(?:-|$)/, 'the changed app shell needs the v272 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*time-field-v272(?:-|["'])/,
    'the current application CSS needs the v272 cache-busting pin');
});
