...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v265';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*inspiration-v265(?:-|$)/, 'the changed app shell needs the v265 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*inspiration-v265(?:-|["'])/,
    'the current application CSS needs the v265 cache-busting pin');
});
