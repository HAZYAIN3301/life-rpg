...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v263';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*attention-cas-v263(?:-|$)/, 'the changed app shell needs the v263 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*critical-v261(?:-|["'])/,
    'the current application CSS needs the v261 cache-busting pin');
});
