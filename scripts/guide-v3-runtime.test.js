...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v281';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*spheres-v281(?:-|$)/, 'the changed app shell needs the v281 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*spheres-v281(?:-|["'])/,
    'the current application CSS needs the v281 cache-busting pin');
});
