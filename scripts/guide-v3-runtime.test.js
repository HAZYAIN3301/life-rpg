...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v284';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*charts-v284(?:-|$)/, 'the changed app shell needs the v284 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*charts-v284(?:-|["'])/,
    'the current application CSS needs the v284 cache-busting pin');
});
