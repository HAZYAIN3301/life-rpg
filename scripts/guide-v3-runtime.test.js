...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v296';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*owner-v296(?:-|$)/, 'the changed app shell needs the v296 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*owner-v296(?:-|["'])/,
    'the current application CSS needs the v296 cache-busting pin');
});
