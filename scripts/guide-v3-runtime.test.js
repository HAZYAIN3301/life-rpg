...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v260';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*critical-v260(?:-|$)/, 'the changed app shell needs the v260 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*critical-v260(?:-|["'])/,
    'the current application CSS needs the v260 cache-busting pin');
});
