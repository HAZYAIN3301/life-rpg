...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v297';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*extension-v297(?:-|$)/, 'the changed app shell needs the v297 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*extension-v297(?:-|["'])/,
    'the current application CSS needs the v297 cache-busting pin');
});
