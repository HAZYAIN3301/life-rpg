...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v208';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*v208(?:-|$)/, 'the current app shell needs the v208 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*v208(?:-|["'])/,
    'the current application CSS needs the v208 cache-busting pin');
});
