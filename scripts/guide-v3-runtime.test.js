...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v244';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*attention-commitment-v244(?:-|$)/, 'the current app shell needs the v244 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*attention-commitment-v244(?:-|["'])/,
    'the current application CSS needs the v244 cache-busting pin');
});
