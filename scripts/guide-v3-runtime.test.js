...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v266';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*devices-v266(?:-|$)/, 'the changed app shell needs the v266 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*devices-v266(?:-|["'])/,
