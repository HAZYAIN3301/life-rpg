...
test('v195 offline shell pins all Guide runtime scripts and locale copies', () => {
  sourceMatches(SW, /const CACHE = 'satoru-v278';/);
...
test('Context pack v205 explicitly releases exact Guide copy and chapter versions for RU/EN/DE/UK/ES', () => {
  ...
  assert.match(appSource, /\?v=[^"']*session-v278(?:-|$)/, 'the changed app shell needs the v278 cache-busting pin');
  sourceMatches(INDEX, /styles\.css\?v=[^"']*session-v278(?:-|["'])/,
    'the current application CSS needs the v278 cache-busting pin');
});
