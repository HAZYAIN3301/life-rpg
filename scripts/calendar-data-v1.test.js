...
test('task writes are blocked at the lowest Store primitive', async () => {
  ...
  let fetches = 0;
  const makePut = Function('fetch', 'taskWriteAllowed', 'console', 'toast', `return (${putSource})`);
  const blockedPut = makePut(async () => { fetches += 1; return { ok: true }; }, () => false, { error() {} }, () => {});
  assert.equal(await blockedPut('tasks', []), false);
  assert.equal(fetches, 0);

  const allowedPut = makePut(async () => { fetches += 1; return { ok: true }; }, () => true, { error() {} }, () => {});
