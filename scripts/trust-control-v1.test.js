...
test('new copy has complete locales and normalized path quotes', () => {
  const keys = [
    ...
    'Мягкий (Доверие) или жёсткий (Контроль). Награды одинаковы; различаются тон и способ разбирать просроченное. Сменить можно в любой момент.',
    'Просроченное: мягкое предложение перенести или отпустить.', 'Обязательство доступно без ставки ресурсами.',
    'Базовые XP и золото не меняются.', 'Оформление: следует твоим настройкам темы и нарратора.',
    'Серия: 0 базовых защит от пути.', 'Просроченное: обязательный явный выбор — уменьшить, перенести или отпустить.',
