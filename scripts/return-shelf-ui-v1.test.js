...
test('интеграция подключает профиль и каталог до UI/app и даёт разделу собственную иконку', () => {
  ...
  const appAt = index.indexOf('app.js?v=20260903-write-fence-v215-4');
  ...
  assert.match(index, /styles\.css\?v=20260903-write-fence-v215-4/);
  assert.match(sw, /satoru-v228/);
  assert.match(app, /PWA_CACHE_VERSION = 'satoru-v228'/);
});
