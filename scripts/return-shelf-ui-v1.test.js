...
test('интеграция подключает профиль и каталог до UI/app и даёт разделу собственную иконку', () => {
  ...
  const appAt = index.indexOf('app.js?v=20260901-browser-companion-v213-1');
  ...
  assert.match(index, /styles\.css\?v=20260901-browser-companion-v213-1/);
  assert.match(sw, /satoru-v214/);
  assert.match(app, /PWA_CACHE_VERSION = 'satoru-v214'/);
});
