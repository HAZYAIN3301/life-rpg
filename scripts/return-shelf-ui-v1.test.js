...
test('интеграция подключает профиль и каталог до UI/app и даёт разделу собственную иконку', () => {
  ...
  const appAt = index.indexOf('app.js?v=20260910-secretary-v254-1');
  ...
  assert.match(index, /styles\.css\?v=20260908-design-v248-1/);
  assert.match(sw, /satoru-v254/);
  assert.match(app, /PWA_CACHE_VERSION = 'satoru-v254'/);
});
