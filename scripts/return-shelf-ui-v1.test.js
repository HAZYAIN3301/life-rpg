...
test('интеграция подключает профиль и каталог до UI/app и даёт разделу собственную иконку', () => {
  ...
  const appAt = index.indexOf('app.js?v=20260911-secretary-v255-1');
  ...
  assert.match(index, /styles\.css\?v=20260911-secretary-v255-1/);
  assert.match(sw, /satoru-v255/);
  assert.match(app, /PWA_CACHE_VERSION = 'satoru-v255'/);
});
