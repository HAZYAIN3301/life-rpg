...
// Dependency-free visual harness for Shadow/Pet scenes v160. It launches the
...
const ORIGIN = process.env.QA_ORIGIN || 'http://127.0.0.1:4393';
const COOKIE_FILE = process.env.QA_COOKIE_FILE || '/private/tmp/satoru-shadow-pet-v160.cookies';
const OUT_DIR = path.resolve(process.env.QA_OUT_DIR || path.join(__dirname, '..', 'docs/design-qa/2026-08-15-shadow-pet-v160'));
const DEBUG_PORT = Number(process.env.QA_CDP_PORT || 9460);
...
async function roomMetrics(cdp) {
  return evaluate(cdp, `(() => {
    ...
    return {
      ...
      shadowActions:[...document.querySelectorAll('.den-shadow-actions button')].map((button)=>(button.textContent||'').trim()),
      scene:rect('.den-scene'), avatar:rect('.den-avatar-core'), body:rect('.den-body-toad'), recovery:rect('.den-recovery-slug'), resources:rect('.den-resources-penguin'), shadow:rect('.den-companion[data-shadow-den]'), pair:rect('.shadow-den-pair-v1.is-active, .den-pet-pair-v1.is-active')
    };
  })()`);
}
...
async function runScenario(cdp, spec) {
  ...
  await evaluate(cdp, `(() => { const scene=document.querySelector('.den-scene'); if(scene)window.scrollTo(0,Math.max(0,scene.getBoundingClientRect().top+window.scrollY-76)); return true; })()`);
...
async function main() {
  ...
  try {
    ...
    const petPairInvoke = `(() => { const shell=document.querySelector('.den-shell'); if(!shell||typeof playPetPairScene!=='function')return false; void playPetPairScene(shell,'body-recovery',{automatic:false,duration:8400}); return true; })()`;
    const scenarios = [
      { label: 'shadow-call', width: 375, height: 812, action: 'shadow-den-pair', mode: 'attune', approachAt: 920, contactAt: 3150 },
      { label: 'pet-recovery-compact', width: 360, height: 800, invoke: petPairInvoke, approachAt: 920, contactAt: 3100 },
      { label: 'pet-recovery-desktop', width: 1280, height: 900, invoke: petPairInvoke, approachAt: 1050, contactAt: 3250 },
    ];
    const report = [];
    for (const spec of scenarios) report.push(await runScenario(cdp, spec));
    for (const item of report) {
      if (item.contact.overflow > 1) throw new Error(`${item.spec.label}: horizontal overflow ${item.contact.overflow}`);
      if (item.contact.minTarget < 41.5) throw new Error(`${item.spec.label}: touch target ${item.contact.minTarget}`);
      if (item.contact.shadowActions.join('|') !== 'Позвать Тень|Поговорить|Проверить курс') throw new Error(`${item.spec.label}: persistent Shadow actions drifted`);
      if (!item.contact.pair || item.contact.pair.display === 'none' || Number(item.contact.pair.opacity) < .9) throw new Error(`${item.spec.label}: authored contact frame is not visible`);
      if (item.spec.label.startsWith('pet-') && item.contact.visibleGround < 1) throw new Error(`${item.spec.label}: every ground resident disappeared`);
    }
